...
- 2026-09-10 build-line hardening: `BuildLineContractTest` now verifies root `java.version`, exact Maven Wrapper/property agreement, Maven 4 external-parent syntax, one test logging provider and public-BOM import of `ddd4j-javalin-dependencies`. This exposed and fixed 7.2.x still declaring Java 17/Maven 3, two 6.7.x POMs missing their Maven namespace, all lines silently using NOP logging in tests, and the public BOM failing to export the line-specific Javalin version.
