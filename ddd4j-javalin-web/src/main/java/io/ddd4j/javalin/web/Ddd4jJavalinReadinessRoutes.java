...
public final class Ddd4jJavalinReadinessRoutes {
    ...
    public static void register(Javalin app, Ddd4jJavalinRuntime runtime) {
        ...
        app.unsafe.routes.get("/health/liveness", context -> context.contentType("application/json")
                .result("{\"status\":\"LIVE\"}"));
        app.unsafe.routes.get("/health/readiness", context -> readiness(context, runtime));
        app.unsafe.routes.get("/health", context -> health(context, runtime));
    }
